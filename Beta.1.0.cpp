...
void reservaUmQuarto(Hotel hotel) {
    ...
    if (cliente > hotel.HotelIdCliente) {
        ...
        system("pause");
        Resevas(hotel);
    } else if (hotel.clientes[cliente].temQuarto == 1) {
    ...
    char buffer[100];
    sprintf(buffer, "Numero do quarto %d / Nome do cliente: %s / Preço do quarto %.2f / Data da reserva %d/%d/%d", hotel.quartos[numero].numero, hotel.clientes[cliente].nome, hotel.quartos[numero].preco, hotel.tempo.tm_mday, hotel.tempo.tm_mon, hotel.tempo.tm_year);
    strcpy(hotel.historico[hotel.Contador[2]], buffer);
    hotel.Contador[4]++;
